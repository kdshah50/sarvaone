...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && typeof data === "object" && (data as { channel?: string }).channel) {
        params.set("channel", String((data as { channel: string }).channel));
      }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-[#E5E0D8] p-8 shadow-sm">
          ...
          <p className="text-sm text-[#6B7280] mb-6">
            {country === "US"
              ? "We’ll text you a 6-digit code to verify your number. Booking chat stays in Sarvaone after you log in."
              : "Te enviamos un código de 6 dígitos por WhatsApp. El chat con proveedores sigue en Sarvaone después de entrar."}
          </p>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-[#6B7280] block mb-2 tracking-wide">
                {country === "US" ? "MOBILE NUMBER" : "WHATSAPP NUMBER"}
              </label>
            ...
            <button
              ...
              style={{ background: loading ? "#6B7280" : country === "US" ? "#1B4332" : "#25D366" }}
            >
              {loading ? (
                <>
